...
const AppSubmenu = (props) => {
    ...
    const renderLink = (item, i) => {
        ...
        if (item.to && (item.isAdmin || item.isUser || item.isPublication)) {
            return (
    ...
    let items = props.items && props.items.map((item, i) => {
        ...
        if(props.root) {
            if(!item.isAdmin && !item.isUser && !item.isPublication){
            ...
            return (
                <li className={styleClass} key={i} role="none">
                    {props.root === true && <React.Fragment>
                        <div className="layout-menuitem-root-text" aria-label={item.label} style={{fontSize:"16px"}}>
                        <Divider />
                        {item.label}
                        <Divider />
                        </div>
