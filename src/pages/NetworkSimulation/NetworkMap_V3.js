  ...
  var lineData = require('./lineData.json')
    ...
    function showBusToolTip(id, name, generation){
      ...
      for(var k=0; k< Object.keys(generation).length; k++){
        ...
        i.innerHTML = Object.keys(generation)[k]
        i.style.fontWeight = 600
        div.appendChild(i)
        i = document.createElement("i")
        i.innerHTML = " : " + generation[Object.keys(generation)[k]].capacity + " MW"
        div.appendChild(i)
      }
      return(div)
    }
    useEffect(() => {
      ...
      for (var i=0; i<Object.keys(busData.data).length; i++){
        for (var j=0; j< Object.keys(busData.data[Object.keys(busData.data)[i]].generation).length; j++){
          if(busData.data[Object.keys(busData.data)[i]].generation[Object.keys(busData.data[Object.keys(busData.data)[i]].generation)[j]].checked){
            base[Object.keys(busData.data[Object.keys(busData.data)[i]].generation)[j]] += busData.data[Object.keys(busData.data)[i]].generation[Object.keys(busData.data[Object.keys(busData.data)[i]].generation)[j]].capacity
          }
          
        }
      }
      const total_generation = Object.values(base).reduce((partialSum, a) => partialSum + a, 0)
      const a = [
        {"type": "Natural Gas", "fleet": base["Natural Gas"]},
        {"type": "Hydro (Dam)", "fleet": base["Hydro (Dam)"]},
        {"type": "Hard Coal", "fleet": base["Hard Coal"]},
        {"type": "Import Coal", "fleet": base["Import Coal"]},
        {"type": "Lignite", "fleet": base["Lignite"]},
        {"type": "Solar", "fleet": base["Solar"]},
        {"type": "Wind", "fleet": base["Wind"]} 
      ]
    ...
    return (
      <div className="col-12">
        ...
        <Grid columns={2} relaxed='very' stackable>
          <Grid.Column>
            <div id="map" style={{ height: "70vh", width: "75vw" }}>
            </div>  
          </Grid.Column>
