...
import { EpiasService } from './EpiasService';
const EpiasData = () => {
    const epiasService = new EpiasService()
    ...
    */
    ...
    const execute = async () => {
      ...
      */
      ...
      const res = await epiasService.executee(data)
      if (res.success){
        SetResults(JSON.parse(res.object.object.body.result))   
        Setisloading(false)    
      }
    }
    const changeDate = (val) =>{
      if(val){
        setStartDate(val[0].getFullYear() + "-" + ((val[0].getMonth()+1) <10? "0" + (val[0].getMonth()+1) : val[0].getMonth()+1) + "-" + (val[0].getDate() <10? "0" + val[0].getDate() : val[0].getDate()))
        setEndDate(val[1].getFullYear() + "-" + ((val[1].getMonth()+1) <10? "0" + (val[1].getMonth()+1) : val[1].getMonth()+1) + "-" + (val[1].getDate() <10? "0" + val[1].getDate() : val[1].getDate()))
      }
      else{
        setStartDate("")
        setEndDate("")
    ...
    <div className="card">
        <h4 style={{fontFamily:"Arial" , fontWeight:"bold",fontSize:"20px"}}>Export Data from EPİAŞ Transparency Platform</h4>
        <div className='grid' >
          <div style={{width:"30%", height:"500"}}>
            <div className="p-fluid">
              <div className="field" style={{paddingBottom:"10px", paddingTop:"20px"}}>
                <label style={{fontSize:"18px" , fontFamily:"Arial"}}>Data <img src="./info_V1.png" title="Info if necessary"/></label>
              ...
              <div> 
                <label style={{fontSize:"18px" , fontFamily:"Arial"}}> Date Range <img src="./info_V1.png" title="Info if necessary"/></label>
                <br></br>
                <DateRangePicker id="Date-Range" size='lg' placeholder="Select Date Range" ranges={[{}]} disabledDate={date => isAfter(date, new Date())} showOneCalendar style={{width:"75%"}} onChange={(e) => changeDate(e)}/>
              </div> 
              <div style={{paddingTop:"250px"}}>
                <Divider align='right'>
                    <Button label="Execute" disabled={!data_selected.code || startDate===""} style={{color:"white"}}  icon="pi pi-search" className="p-button-outlined" onClick={execute}></Button>
                </Divider>
              </div>                    
            </div>   
          </div> 
